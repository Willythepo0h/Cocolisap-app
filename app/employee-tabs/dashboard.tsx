...
export default function EmployeeDashboardScreen() {
  const [lastestTimestamp, setLatestTimestamp] = useState(new Date());
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const sortedHistory = data.cocolisap_history?.slice().sort(
        (a: AvgEntry, b: AvgEntry) =>
          new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()
      );
  
      if (sortedHistory && sortedHistory.length > 0) {
        setLatestTimestamp(new Date(sortedHistory[0].timestamp));
      }
    } catch (err) {
  ...
  const formattedLastUpdate = useMemo(() => {
    if (!lastestTimestamp) return null;

    return new Intl.DateTimeFormat('en-US', {
      dateStyle: 'medium',
      timeStyle: 'short'
    }).format(lastestTimestamp);
  }, [lastestTimestamp]);
        ...
        <ThemedView style={{flexDirection: 'row', width: '100%', justifyContent: 'space-between', alignItems: 'center'}}>
          {formattedLastUpdate && (
            <ThemedText style={{marginTop: 10, alignSelf:'flex-start'}}>
              Last Update: <ThemedText type="default">{formattedLastUpdate}</ThemedText>
            </ThemedText>
          )}
...
const styles = StyleSheet.create({
  ...
  refreshButton: { borderWidth: 1, borderRadius: 10, paddingHorizontal: 5, marginTop: 5}
});
